--[[
  ...
  The script has no optimizing purposes in this current state. But later updates will have more optimizing updates and new changes.
  ...
  Bugs:
  Mesh chams doesnt have double execution protection, so expect bugs with it.
  ...
  Distance
  Flags
  Weapon ESP --supports project delta Holding
  Skeleton ESP -- taken from nocturnal
  Health Bar

  Custom Directories
  Custom stuff
  I dont f*cking know K*d...
...
--// esp
    local ESPConfig = {
        ...
        Players = true,
        LocalPlayer = false,
        LimitFPS = 120, -- Set to 0 to disable limit
        Boxes = false,
        ...
        Distance = {
            ...
            Ending = "m", -- "m" or "s" depending on unit
            Gap = 3,
        },
        ...
        Flags = {
            ...
            Colors = {
                ...
                Swimming = Color3.fromRGB(255, 255, 255),
            }
        },
...
--// variables
    ...
    local function UpdateESPObj(espObj, position, size, name, distanceStuds, instance, isCheap, nonHuman, noStatus, configOverride, onScreen)
        ...
        if GetCfg("HealthBar.Enabled") and instance:IsA("Model") and humanoid then
                ...
                if isHorizontal then
                    ...
                    if hpPos == "Top" then
                        espObj.HealthBarOutline.Position = UDim2.new(0, x - 1, 0, y - o - GetCfg("HealthBar.SideGap") - GetCfg("HealthBar.Width") - 1)
                    else -- Bottom
                    ...
                    if hpPos == "Left" then
                        espObj.HealthBarOutline.Position = UDim2.new(0, x - o - GetCfg("HealthBar.SideGap") - GetCfg("HealthBar.Width") - 1, 0, y - 1)
                    else -- Right
                ...
                if showText then
                    ...
                    espObj.HealthText.TextColor3 = GetCfg("HealthBar.FollowGradientColorText") and healthColor or GetCfg("TextColor")
...
--// logic
    local function Get2DBoundingBox(instance)
        local rootPart
        if instance:IsA("Model") then
            rootPart = instance:FindFirstChild("HumanoidRootPart") or instance:FindFirstChild("Torso") or instance.PrimaryPart or instance:FindFirstChildWhichIsA("BasePart")
    ...
    getgenv().SensoryESP_Loop = RunService.RenderStepped:Connect(function()
        ...
        for inst, data in pairs(TrackedInstances) do
            ...
            if rootPart then
                ...
                UpdateESPObj(data.espObj, pos2d, size2d, data.name, distanceStuds, inst, data.Cheap, data.NonHuman, data.NoStatus, data.Config, onscreen)
            else
                UpdateESPObj(data.espObj, nil, nil, data.name, 0, inst, data.Cheap, data.NonHuman, data.NoStatus, data.Config, false)
            end
        end
    end)
--
